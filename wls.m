...
CvE = diag(inv(H'*inv(Ri)*H)); % Covariance..
CvE = diag([CvE(nbus:end); CvE(1:nbus-1)]); % To get covariance of voltage on first..
% Rotation Matrix..
R11 = diag(cos(del));
R12 = [zeros(1,nbus-1); diag(-V(2:end).*sin(del(2:end)))];
R21 = diag(sin(del));
R22 = [zeros(1,nbus-1); diag(V(2:end).*cos(del(2:end)))];
R = [R11 R12; R21 R22];

W1 = diag(R*CvE*R');  % Covariance in Rectangular co-ordinates..
W1(nbus+1) = 1e-20;
Del = 180/pi*del;
E2 = [V Del]; % Bus Voltages and angles..
n = length(V);
disp('---------------- State Estimation without PMUs -----------------------------------------');
disp('--------------------------');
disp('| Bus |    V   |  Angle  | ');
disp('| No  |   pu   |  Degree | ');
disp('--------------------------');
